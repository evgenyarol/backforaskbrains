...
module.exports = class LinkedInRestClient {
    ...
    constructor(
        ...
        if (!this.client_id)
            throw new Error('Missing "client_id" during initialization!');
        if (!this.client_secret)
            throw new Error('Missing "client_secret" during initialization!');
        if (!this.redirect_uri)
            throw new Error('Missing "redirect_uri" during initialization!');
    }

    /**
     * Retrive current member's email
     *
     * @param access_token
    ...
    async getCurrentMemberEmail(access_token) {
        ...
        return this.invoke("GET", url, undefined, undefined, {
            withAuth: true,
            access_token
        });
    }
    ...
    /**
     * Retrieve current member's picture
     *
     * @param access_token
     * @returns {Promise<Object>}
     */
    async getCurrentMemberPicture(access_token) {
        if (!access_token) throw new Error("Access code cannot be empty");
        const url = `${this.url}/me?projection=(profilePicture(displayImage~:playableStreams))`;
        return this.invoke("GET", url, undefined, undefined, {
            withAuth: true,
            access_token
        });
    ...
    async postShareFromAccount(body, access_token) {
        ...
        return this.invoke("POST", url, undefined, body, {
            withAuth: true,
            access_token
        });
    }
    ...
    async getOrganizationAdmins(organization_urn) {
        if (!organization_urn)
            throw new Error('Parameter "organization_urn" cannot be empty');
    ...
    generateOptions(method, url, headers, body, auth) {
        ...
        if (auth.withAuth) {
            if (!auth.access_token)
                throw new Error(
                    'Missing required "access_token" in the auth body'
                );
